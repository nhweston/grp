...
import org.bitbucket.nhweston.grp.Field
import org.bitbucket.nhweston.grp.Field._

class Mat[Y <: Int, X <: Int, T] private (val elems: Array[T]) (implicit y: ValueOf[Y], x: ValueOf[X]) {
...
object Mat {
    ...
    implicit def mkField[D <: Int, T] (implicit d: ValueOf[D], field: Field[T]) : Field[Mat[D, D, T]] = {
        ...
        new Field[Self] {
            override def plus (x: Self, y: Self) : Self = Mat ((x.elems zip y.elems) .map {case (a, b) => a + b})
            ...
            override def negate (x: Self) : Self = Mat (x.elems.map (-_))
            override def invert (x: Self): Self = ???
            override lazy val zero: Self = Mat (Seq.fill (size) (0))
