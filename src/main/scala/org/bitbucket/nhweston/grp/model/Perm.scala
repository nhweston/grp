...
class Perm[M <: Int] private (f: Seq[Int]) (implicit m: ValueOf[M]) extends (Int => Int) {
    ...
    def cyc[M <: Int] (cycle: Int*) (implicit m: ValueOf[M]) : Perm[M] = {
        (cycle.map (math.floorMod (_, m.value))) match {
            case cycle @ (head +: tail) =>
                ...
                new Perm ((0 until m.value) .map (n => fMap.getOrElse (n, 0)))
            case Nil => new Perm (0 until m.value)
        }
    }

    def apply[M <: Int] (f: Int => Int) (implicit m: ValueOf[M]) : Perm[M] = {
        new Perm ((0 until m.value) .map (n => math.floorMod (f (n), m.value)))
    }
